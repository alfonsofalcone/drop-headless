...
export default function Header() {
  ...
  const toggleMenu = () => {
    ...
    document.querySelector("html").style.overflowY = menuOpen
      ? "auto"
      : "hidden";
  };
  ...
  return (
    <header className="absolute top-0 w-full p-6 lg:p-10">
      <div className="header-menu flex items-center justify-between">
        ...
        <div
          className={`header-menu-button fixed right-0 mr-6 lg:mr-10 z-50 cursor-pointer flex items-center justify-center w-10 h-10 md:w-14 md:h-14 bg-drop-gray rounded-full ${
            menuOpen ? "open" : ""
          }`}
          onClick={toggleMenu}
        >
          <div className="block w-4 md:w-5 lg:w-6 h-0.5 bg-black"></div>
          <div className="block w-4 md:w-5 lg:w-6 h-0.5 bg-black"></div>
        </div>
        {/* Pannello del menu */}
        <div
          className={`header-menu-panel fixed top-0 left-0 w-full h-full max-w-full p-6 lg:p-10 ${
            menuOpen
        ...
        >
          ...
          {menuItems.map((item, index) => (
            <div
              key={`menu-item-${index}`}
              className={`header-menu-lists flex flex-col w-full h-full md:w-drop-header-menu-lists gap-y-6 ml-auto py-10`}
            >
              {item.subMenuItems?.map((subItem, subIndex) => (
                <div
                  key={`sub-menu-item-${subIndex}`}
                  className={`header-menu-list flex grow-0 shrink bg-drop-${
                    subItem.subMenuBgColor
                  } ${
                    subItem.isFooterItem
                      ? "footer-menu basis-auto"
                      : "basis-full"
                  }`}
                >
                  <ul
                    className={`${subItem.isFooterItem ? "language-list" : ""}`}
                  >
                  ...
                  {subItem.showMinorLinks && (
                    <ul
                      className={`${
                        subItem.isFooterItem ? "social-list" : "sub-list"
                      }`}
                    >
                      {subItem.subMenuMinorLinks?.map(
                        (subMenuMinorLink, subLinkMinorIndex) => (
                          <li key={`sub-menu-link-${subLinkMinorIndex}`}>
                            <Link
                              href={`${subMenuMinorLink.subMenuMinorLinkUrl}`}
                            >
